...
typedef struct {  /* GPS/QZS/GAL broadcast ephemeris type */
  /* GPS/QZS/GAL/BDS/IRN 广播星历（从 RINEX .n 导航文件解码而来） */
  int sat;        /* 卫星编号（含系统信息，如 1-32=GPS；见 satno()/satsys() 编解码） */
  int iode, iodc; /* IODE: 星历数据期号——星历内容变化时自增，用于判断星历是否已更新 */
                  /* IODC: 钟差数据期号——用于校验钟差参数是否与轨道参数匹配 */
  int sva;        /* 卫星精度（URA 指数，User Range Accuracy 等级），越小精度越高，用于定权/RAIM */
  int svh;        /* 卫星健康标志（0: 健康可用，非 0: 不可用，定位前应排除） */
  int week;       /* 周数：GPS/QZS 用 GPS 周，GAL 用 Galileo 周（与 toes 合成精确 Toe 时刻） */
  int code;       /* GPS/QZS: L2 上采用的码（C/A 或 P） */
                  /* GAL: 数据源（按 RINEX 3.03 定义） */
  /* BDS: 数据源（0:未知,1:B1I,2:B1Q,3:B2I,4:B2Q,5:B3I,6:B3Q） */
  int flag;       /* GPS/QZS: L2 P 数据标志 */
                  /* BDS: 导航类型（0:未知,1:IGSO/MEO,2:GEO——GEO 需特殊处理） */
  gtime_t toe, toc, ttr;
                  /* toe: 轨道参考时刻（Toe）——开普勒参数在该时刻成立 */
                  /* toc: 钟差参考时刻（Toc）——钟差多项式 af0/af1/af2 在该时刻成立 */
                  /* ttr: 电文传输时刻（T_trans）——卫星发射该帧星历的时间 */
                  /* 三个时刻均为 GPST；Toe 与 Toc 通常相同，但也可不同 */
                  /* —— 开普勒轨道参数（均在参考时刻 toe 处取值）—— */
  double A, e, i0, OMG0, omg, M0, deln, OMGd, idot;
                  /* A   : 轨道半长轴的平方根 sqrt(a)（注意！存的是 sqrt(a) 而非 a，
                           代码里用 a=A*A 还原半长轴）[sqrt(m)] */
                  /* e   : 轨道偏心率（无量纲，0~1） */
                  /* i0  : 轨道倾角 [rad] */
                  /* OMG0: 升交点赤经（参考历元时刻）[rad] */
                  /* omg : 近地点角距 [rad] */
                  /* M0  : 平近点角（参考历元时刻）[rad] */
                  /* deln: 平均角速度改正量 Δn [rad/s] */
                  /* OMGd: 升交点赤经变化率 [rad/s] */
                  /* idot: 轨道倾角变化率 [rad/s] */
  double crc, crs, cuc, cus, cic, cis;
                  /* 轨道摄动的调和改正项（以升交角距 u 为自变量，二阶谐波模型） */
                  /* crc/crs: 径向改正，余弦/正弦 [m] */
                  /* cuc/cus: 升交角距改正，余弦/正弦 [rad] */
                  /* cic/cis: 轨道倾角改正，余弦/正弦 [rad] */
  double toes;    /* toe 在本周内的秒数 [s]（week + toes 合成精确的 Toe 时刻） */
  double fit;     /* 星历拟合间隔（fit interval）[h]，决定星历可靠外推的时间范围 */
  double f0, f1, f2;
                  /* 卫星钟差多项式系数（相对参考时刻 toc，二阶多项式） */
                  /* f0=af0: 钟差 [s]；f1=af1: 钟漂 [s/s]；f2=af2: 钟漂变化率 [s/s^2] */
                  /* 注意：该钟差是"码"钟差，已隐含码硬件延迟（见笔记 1.1 的 d 项讨论） */
  double tgd[6];  /* 群延迟参数（各星座含义不同，单位 s），用于扣除频点间码延迟差异 */
                  /* GPS/QZS: tgd[0]=TGD（L1/L2 间群延迟） */
                  /* GAL: tgd[0]=BGD_E1E5a, tgd[1]=BGD_E1E5b */
                  /* CMP: tgd[0]=TGD_B1I,  tgd[1]=TGD_B2I/B2b, tgd[2]=TGD_B1Cp */
                  /*      tgd[3]=TGD_B2ap, tgd[4]=ISC_B1Cd,    tgd[5]=ISC_B2ad */
  double Adot, ndot;
                  /* CNAV 新增的轨道参数变化率（GPS L2C/L5 等新一代电文） */
                  /* Adot: sqrt(a) 随时间的变化率 [sqrt(m)/s]；ndot: Δn 的变化率 [rad/s^2] */
} eph_t;

typedef struct {     /* GLONASS broadcast ephemeris type */
  /* GLONASS 广播星历——与 GPS 不同：不给开普勒参数，直接给位置/速度/加速度 */
  int sat;           /* 卫星编号（含系统信息；GLONASS 用槽位号表示） */
  int iode;          /* IODE（tb 字段的 0-6 位）——星历数据期号，用于校验星历更新 */
  int frq;           /* 卫星频率号——GLONASS 是频分多址（FDMA），每颗卫星 L1/L2
                        实际频率 = 基频 + 频率号 × 偏频，见 FREQ1_GLO/DFRQ1_GLO */
  int svh, sva, age; /* svh: 健康标志；sva: 精度等级；age: 卫星服役年限 */
  gtime_t toe;       /* 星历参考时刻（GPST）——pos/vel/acc 在该时刻成立 */
  gtime_t tof;       /* 电文帧时刻（GPST）——卫星发射该帧星历的时间 */
  double pos[3];     /* 卫星位置（ECEF）[m] */
  double vel[3];     /* 卫星速度（ECEF）[m/s] */
  double acc[3];     /* 卫星加速度（ECEF）[m/s^2]（用于外推到相邻时刻） */
  double taun, gamn; /* taun: 卫星钟差 [s]；gamn: 相对频率偏差（无量纲） */
  double dtaun;      /* L1 与 L2 之间的信号延迟差 [s] */
} geph_t;
...
typedef struct {     /* navigation data type */
  /* 导航数据总集：汇聚从导航文件/电文解码出的星历、钟差、UTC、电离层、DCB 等
     是所有定位函数（satposs/rtkpos/pppos 等）读取导航信息的统一入口 */
  int n, nmax;       /* GPS/QZS/GAL/BDS/IRN 广播星历的数量/已分配容量 */
  int ng, ngmax;     /* GLONASS 广播星历的数量/已分配容量 */
  int ns, nsmax;     /* SBAS 星历的数量/已分配容量 */
  int ne, nemax;     /* 精密星历（SP3）的数量/已分配容量 */
  int nc, ncmax;     /* 精密钟差的数量/已分配容量 */
  int na, namax;     /* 历书（almanac）的数量/已分配容量 */
  int nt, ntmax;     /* TEC 格网数据的数量/已分配容量 */
  eph_t *eph;        /* GPS/QZS/GAL/BDS/IRN 广播星历数组（动态分配） */
  geph_t *geph;      /* GLONASS 广播星历数组 */
  seph_t *seph;      /* SBAS 星历数组 */
  peph_t *peph;      /* 精密星历数组（SP3 文件） */
  pclk_t *pclk;      /* 精密钟差数组 */
  alm_t *alm;        /* 历书数组（粗略轨道，用于快速捕获） */
  tec_t *tec;        /* TEC 格网数据（电离层延迟，IONEX 文件） */
  erp_t erp;         /* 地球自转参数 ERP（极移 xp/yp、UT1-UTC，精密定位用） */
  double utc_gps[8]; /* GPS 的 delta-UTC 参数
                        {A0,A1,Tot,WNt,dt_LS,WN_LSF,DN,dt_LSF}——用于 GPST↔UTC */
  double utc_glo[8]; /* GLONASS UTC 参数 {tau_C,tau_GPS} */
  double utc_gal[8]; /* Galileo UTC 参数 */
  double utc_qzs[8]; /* QZSS UTC 参数 */
  double utc_cmp[8]; /* 北斗 UTC 参数 */
  double utc_irn[9]; /* IRNSS UTC 参数 {A0,A1,Tot,...,dt_LSF,A2} */
  double utc_sbs[4]; /* SBAS UTC 参数 */
  double ion_gps[8]; /* GPS 电离层模型参数（Klobuchar）{a0,a1,a2,a3,b0,b1,b2,b3} */
  double ion_gal[4]; /* Galileo 电离层参数 {ai0,ai1,ai2,0} */
  double ion_qzs[8]; /* QZSS 电离层模型参数 */
  double
      ion_cmp[8]; /* 北斗电离层模型参数 {a0,a1,a2,a3,b0,b1,b2,b3} */
  double ion_irn[8]; /* IRNSS 电离层模型参数 */
  int glo_fcn[32];   /* GLONASS 频率号（FCN+8，下标为 GLONASS 槽位），用于频分多址定频 */
  double cbias[MAXSAT][3];    /* 卫星差分码偏差 DCB [m]
                                 (0:P1-P2, 1:P1-C1, 2:P2-C2) */
  double rbias[MAXRCV][2][3]; /* 接收机差分码偏差 DCB [m]（每接收机每对码） */
  pcv_t pcvs[MAXSAT];         /* 卫星天线相位中心（PCV）改正参数 */
  sbssat_t sbssat;            /* SBAS 卫星改正（快/长期轨道钟差改正） */
  sbsion_t sbsion[MAXBAND + 1]; /* SBAS 电离层改正（IGP 格网） */
  dgps_t dgps[MAXSAT];          /* DGPS 伪距改正（基准站播发） */
  ssr_t ssr[MAXSAT];            /* SSR 实时改正（轨道/钟差/码相位偏差） */
} nav_t;
...
typedef struct { /* solution type */
  /* 定位解算结果（每个历元的输出，写 .pos 文件） */
  gtime_t time;  /* 解算时刻（GPST） */
  double rr[6];  /* 位置/速度 [m | m/s] */
                 /* 顺序：{x,y,z,vx,vy,vz}（ECEF）或 {e,n,u,ve,vn,vu}（ENU），
                    由 type 字段决定；速度段仅在启用输出速度(outvel)时有效 */
  float qr[6];   /* 位置方差/协方差 [m^2] */
                 /* 顺序：{c_xx,c_yy,c_zz,c_xy,c_yz,c_zx}（ECEF）或 */
                 /*      {c_ee,c_nn,c_uu,c_en,c_nu,c_ue}（ENU）——对角+上三角 */
  float qv[6];   /* 速度方差/协方差 [m^2/s^2]（对应 rr 的速度段） */
  double dtr[6]; /* 接收机钟差（相对各时间系统）[s] */
                 /* dtr[0]=GPS，dtr[1]=GLO-GPS，dtr[2]=GAL-GPS 等系统间偏差 */
  uint8_t type;  /* 坐标系类型（0:xyz-ECEF，1:enu-基线） */
  uint8_t stat;  /* 解状态（SOLQ_???: 0=none,1=fix,2=float,3=sbas,4=dgps,
                    5=single,6=ppp）——定位结果可信度的首要标志 */
  uint8_t ns;    /* 参与解算的有效卫星数 */
  float age;     /* 差分龄期（改正数相对观测的时刻差）[s]，越大可靠性越低 */
  float ratio;   /* 模糊度固定 ratio 检验值（最优/次优整数解距离比），越大越可信 */
  float thres;   /* AR ratio 检验阈值——ratio >= thres 才接受固定解 */
} sol_t;
...
typedef struct {        /* RTCM control struct type */
  ...
  uint32_t nmsg3[400]; /* message count of RTCM 3
                          (1-299:1001-1299,300-329:4070-4099,0:ohter) */
...
typedef struct {     /* processing options type */
  ...
  double prn[6];   /* process-noise std [0]bias,[1]iono [2]trop [3]acch [4]accv
                      [5] pos */
...
typedef struct {                /* RINEX options type */
  ...
  double shift[7][MAXOBSTYPE];  /* phase shift (cyc)
